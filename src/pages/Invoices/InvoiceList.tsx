...
const InvoiceList: React.FC<InvoiceListProps> = ({ onViewChange, invoiceItems, setInvoiceItems, onPrintInvoice, onEditInvoice, onPrintList }) => {
  ...
  return (
    <div className="min-h-full p-6 space-y-5" style={{ background: 'transparent' }}>

      {/* ── Page Header ── */}
      <PageHeader
        title={terms.title}
        subtitle={`${filtered.length} ${terms.countLabel} found · Last updated just now`}
        actions={
          <div className="flex items-center gap-2.5">
            ...
            <div className="relative flex items-center border border-slate-200 rounded-lg bg-white shadow-sm hover:border-slate-300 transition-colors select-none header-dropdown-container">
              <button
                type="button"
                onClick={() => {
                  const normType = typeFilter === 'Sale Return' ? 'Sales Return' : 'Sales Invoice';
                  const activeT = templates.find(t => t.is_active && t.document_type === normType) || templates[0];
                  onPrintList?.(filtered, 'invoices', activeT?.template_id);
                }}
                  ...
                  >
                    <button
                      ...
                      onClick={() => {
                        ...
                        const normType = typeFilter === 'Sale Return' ? 'Sales Return' : 'Sales Invoice';
                        const activeT = templates.find(t => t.is_active && t.document_type === normType) || templates[0];
                        onPrintList?.(filtered, 'invoices', activeT?.template_id, true);
                      }}
