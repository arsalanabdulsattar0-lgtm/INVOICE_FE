...
export const BPAdjustmentEditor: React.FC<Props> = ({
  ...
  return (
    <div className="min-h-full p-6 space-y-6" style={{ background: brand.mainBg }}>
      {/* Page Header */}
      <PageHeader
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        <Card className="p-4" style={{ background: brand.cardBg, borderColor: brand.border, boxShadow: 'none' }}>
        ...
        <div className="rounded-2xl border overflow-hidden"
          style={{ background: brand.cardBg, borderColor: brand.border, boxShadow: 'none' }}>
          {/* Solid Header Bar */}
          ...
          <ScrollArea className="w-full h-[calc(100vh-320px)] overflow-auto" style={{ maxHeight: 'none' }}>
            <table className="w-full min-w-[1150px] table-fixed border-collapse text-left">
              <thead>
                <tr className="border-b select-none" style={{ background: brand.surface, borderColor: brand.border }}>
                  <th style={{ width: '3%', color: brand.textPrimary }} className="pl-4 pr-2 py-3.5 text-center text-[10px] font-black tracking-widest">#</th>
                  <th style={{ width: '14%', color: brand.textPrimary }} className="px-3 py-3.5 text-[10px] font-black tracking-widest">Business Partner No.</th>
                  <th style={{ width: '10%', color: brand.textPrimary }} className="px-3 py-3.5 text-[10px] font-black tracking-widest">Date</th>
                  <th style={{ width: '7%', color: brand.textPrimary }} className="px-3 py-3.5 text-[10px] font-black tracking-widest">Voucher No</th>
                  <th style={{ width: '9%', color: brand.textPrimary }} className="px-3 py-3.5 text-[10px] font-black tracking-widest">GL Account</th>
                  <th style={{ width: '12%', color: brand.textPrimary }} className="px-3 py-3.5 text-[10px] font-black tracking-widest">Narration</th>
                  <th style={{ width: '7%', color: brand.textPrimary }} className="px-3 py-3.5 text-right text-[10px] font-black tracking-widest">Debit (Rs.)</th>
                  <th style={{ width: '7%', color: brand.textPrimary }} className="px-3 py-3.5 text-right text-[10px] font-black tracking-widest">Credit (Rs.)</th>
                  <th style={{ width: '10%', color: brand.textPrimary }} className="px-3 py-3.5 text-[10px] font-black tracking-widest">Tax ID</th>
                  <th style={{ width: '10%', color: brand.textPrimary }} className="px-3 py-3.5 text-left text-[10px] font-black tracking-widest">Tax Amt</th>
                  <th style={{ width: '7%', color: brand.textPrimary }} className="px-3 py-3.5 text-[10px] font-black tracking-widest">Balance</th>
                  <th style={{ width: '4%', color: brand.textPrimary }} className="pl-2 pr-8 py-3.5 text-center text-[10px] font-black tracking-widest">Delete</th>
                </tr>
              </thead>
              <tbody className="overflow-visible">
                {rows.map((row, idx) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b hover:bg-[color:var(--brand-surface)] transition-colors ${
                        isRowActive ? 'bg-[color:var(--brand-primary-10)]' : ''
                      }`}
                      style={{ borderColor: brand.border }}
                    >
                      {/* Active Row Indicator */}
                      <td className="pl-4 pr-2 py-3 text-center text-[12px] font-medium" style={{ color: brand.textSecondary }}>
                        {isRowActive ? (
                          <span className="font-extrabold text-sm" style={{ color: brand.primary }}>✏️</span>
                        ) : (
                          idx + 1
                        )}
                      </td>
                      ...
                      <td className="px-2 py-3 relative overflow-visible" ref={isRowActive ? dropdownRef : undefined}>
                        ...
                        {openDropdownIdx === idx && (
                          <div
                            className="absolute left-2 mt-1 rounded-xl shadow-xl z-[999] max-h-56 overflow-y-auto text-left border-solid w-[320px]"
                            style={{ background: brand.cardBg, borderColor: brand.border, borderWidth: 1, borderStyle: 'solid' }}
                          >
                            <table className="w-full text-xs border-collapse">
                              <thead>
                                <tr className="border-b select-none text-[10px] font-black uppercase tracking-wider"
                                  style={{ background: brand.surface, borderColor: brand.border, color: brand.textSecondary }}>
                              ...
                              <tbody>
                                {filteredDropdownOptions.length === 0 ? (
                                  <tr>
                                    <td colSpan={2} className="p-3 text-center font-semibold" style={{ color: brand.textSecondary }}>
                                      No partners found
                                    </td>
                                  </tr>
                                ) : (
                                  filteredDropdownOptions.map((p: any) => (
                                    <tr
                                      ...
                                      className="border-b cursor-pointer transition-colors hover:bg-[color:var(--brand-surface)]"
                                      style={{ borderColor: brand.border }}
                                    >
                                      <td className="p-2 pl-3 font-black text-[11px]" style={{ color: brand.primary }}>{p.id}</td>
                                      <td className="p-2 font-extrabold text-[11px]" style={{ color: brand.textPrimary }}>{p.rawName}</td>
                                    </tr>
                                  ))
                                )}
                              </tbody>
                      ...
                      <td className="px-2 py-3">
                        <Input
                          ...
                          style={{ background: brand.surface, color: brand.textPrimary }}
                        />
                      </td>
                ...
                <tr
                  onClick={handleAddRow}
                  className="border-b hover:cursor-pointer font-bold"
                  style={{ borderColor: brand.border, background: brand.surface, color: brand.textSecondary }}
                >
                  <td className="pl-4 pr-2 py-3 text-center text-sm font-black" style={{ color: brand.textSecondary }}>*</td>
                  <td colSpan={11} className="pl-3 pr-4 py-3 text-[10.5px] font-black italic select-none" style={{ color: brand.textSecondary }}>
